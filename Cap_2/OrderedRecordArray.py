...
class OrderedRecordArray(object):
    ...
    def merge(self, other_array):
        i, j = 0 ,0
        merged = OrderedRecordArray(len(self) + len(other_array), key=self.__key)
        
        while i < len(self) and j < len(other_array):
            if self.__key(self.__a[i]) < self.__key(other_array.__a[j]):
                merged.insert(self.__a[i])
                i += 1
            else:
                merged.insert(other_array.__a[j])
                j += 1
                
        while i < len(self):
            merged.insert(self.__a[i])
            i += 1 
        while j < len(other_array):
            merged.insert(other_array.__a[j]) 
            j += 1
